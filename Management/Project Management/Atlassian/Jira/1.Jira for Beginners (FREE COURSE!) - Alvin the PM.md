...
**Project**: developing an  software application (will follow the scrum approach to develop the same)
...
Here in the backlog screen, the epics do not immediately appear. because, they reside within the epics panel. To open an epic panel,
...
> Note: Epics can be created overlappingly  - by checking a box at the bottom left -> Create another issue. Epics get created continuously.

Project: Hexr-Daily
1) **Team Lead / Heads Update**
2) **Dwinzo**
- Frontend 
- Backend
3) **Dwinzo IoT**
...
Five epics or five issues for our project and now we can see that we have populated these issues within our project.  we have
- shopping cart 
- cancelling orders 
- user account 
- shopping interface 
...
Let's take a closer look and navigate the same in brief. In the epic section, an epic will be placed with a an arrow, color mark and epic's name.
![[Pasted image 20241108134756.png]]
Click on the arrow, will expand the epic with more details. you'll see is underneath it we see the 
- key for the Epic 
- how many issues are associated with this epic
- how many have been completed 
- an estimate of the number of story points that are required to complete this epic 
>let's click on view details and what we're going to have is a separate panel open up on the right hand side 

**This will show all the details that we've entered in so far for this epic**. 
![[Pasted image 20241108135213.png]]
in the top row of icons,
- we can attach a file to this epic 
- we can add a child issue underneath this epic 
- we can link an issue
- another website or page or even Confluence 
![[Pasted image 20241108142045.png]]
and right below it 
- we have the status of this epic and right now it's showing -todo, then
- we have the description box and below that, we have
- details tied to this epic telling who the person is assigned to the reporter or The person who created it, labels and its priority

![[Pasted image 20241108142138.png]]

there are other fields below this as well, we have
- story points 
- original estimate
- time tracking
- components
- Sprint and
- fix versions 
> All these issues whether it's a story, a task or a bug will have similar fields as same the EPIC.
+all these epic can be color box for identification in ease.
 *I encourage you to color code your epics so that it's easier for you to visually identify your epics and group them together* 
 
so we've color coded for all the EPICs and it's a little more visually easier to identify which one is associated with a specific functionality and categorize each epic based upon its category.

Before we start creating our issues within our project, let's go ahead and create the user stories for our epics 

#### How to create a User Story in Jira
Starting with our first epic for one. back to our Jira project, 
**To create an issue.**
1) can either click the blue **create** button at the top of your screen or 
>2) ==**keyboard shortcut `C`  for user story pop-up  - To create user-stories**==
so inside this dialogue box. 
![[Pasted image 20241108160811.png]]


you can see that we're underneath the project for the app and the issue type we seek to create right now: story.
Project - Project Name
Issue Type: Story
Status: Todo (since we haven't yet started the Sprint)
Summary - User Story in brief
==**Parent - Epic name**==  (very important - this where we choose the EPIC for our user story)
Description: What needs to be developed in order to satisfy the user story.
(to define the acceptance criteria that you'll agree upon with your team)

All done! **Create** button. 
We can see the new user story that we wrote has been added inside of our backlog. 

Will see how to work with the same and what each part of this row is inside of the back log in brief:
- the first icon, the box - indicates what type of issue it is, here its represents **USER STORY.** next,
- Issue Key - is the project key itself.
- Issue Summary, next to that we have
- parent, the issue that falls under the relevant audience
- Status
- Story points - number of story points that presumably gets bloomed to -(typically do during your Sprint planning meeting)
- Priority - Higher, high, low, lower
- Person that's assigned to 
- More options
 To view more on that, left click the summary of that issue. it'll collapse to expand the story in brief.
and If you want to open up the story in a separate tab in brief - Click on the story after right side pop-up.
![[Pasted image 20241108180935.png]]

Now, we can see all of the user-story in a much larger viewing window.
You'll see everything that 
- we entered in for the description. so, the 
- acceptance criteria - User story, activities, comments history of this user story, worklog -  on the left and
- the details, authority and more fields - on the right

Will create more user story of our project and will cover the same creating issues.

Back to backlog,
note: none of the EPICs are selected. since we are creating the Issue stand alone and will be associate to an EPIC later,

> [!NOTE] Scenario: You have attended the stakeholders meeting and collected all the requirements. 
> Now the work is to continuously write down each of the requirement as user story. by,
> Click on **==+ Create Issue button==** and add in the next user story for reviewing items in the cart 
> 
> Then review each one of that stories and associate with the relevant EPICs to translate ALL THE REQUEST 

 ![[Pasted image 20241109104825.png]]

Create Issue and write the User Story in and click enter. 
(here, that you'll have to fill in any of the other details as the last one at a later time period. Such thing that we still do have to assign the EPIC and  you will have to go back to each issue and add in the appropriate descriptions and details at a later time) 
**this is a very quick way to create issues.**
Now, will input the remaining user stories that are a part of our project. 

**Add all that user stories** for the value to product or feature translation journey.

#### Adding Bugs & Subtasks in Backlog - Jira
**Let's add in a few bugs to the project so we can also visualize how this would look during a Sprint** 

Creating a bug is as same as creating an issue but just simply changing the type. 
> Simply just enter the input and Change the issue type box in the front to change the issue type.
![[Pasted image 20241111103635.png]]

Now we've entered user stories and bug issue types for our project . 

###### Subtasks: If a user story can be deconstructed to multiple tasks that we want to keep track and take care of!
eg: let's say that for our very first user story **for checking out items from the shopping cart** - there are tasks like,
- Product Cart
- Payment Gateway
- Order Management
- Order Tracking

To work with such case, will expand the same on a full tab by
> **Open Issue -> Open in new tab -> Add -> Create Subtasks**

we have several subtest that we want to keep track. add those subtests to this user storyby Add -> Subtask button, which is right below the user Story summary line. After that, 
separate line will appear for subtask. Now input all the subtests that associated that to complete this user story.  

**We have a full  list of subtasks which are tied directly to this user story**. Back to baocklog -> Open that user story that we assigned subtasks to. 
> we can see all the sub test that we just inputed on the right hand panel 

Do the same for each of the USER STORIES. 
add another group of subtask for our rest of the user stories, open this up in a separate tab. a little bit easier for us to type in and and enter in all of his subtasks.  so let's do that right now all right so with that completed we have now inputed all the different subtests for this third user story so with that completed let's go back to our

SINCE WE CREATE ALL THESE ISSUE, WE DIDN'T ASSOCIATE IT TO ANY OF THE APPROPRIATE EPICS. 
so we need to take care of that right now. -> to assign a user story to an epic 

#### Assigning Epics to Issues
Simply, 
> ==**1) Drag and Drop each of it or Select the same in multiples and drop it to the  EPICS at the left**==
> ==**2) There will be a '+Epic' button on each of the User story, assign it each or in multiples**==
now we've assigned all of the appropriate issues to their corresponding epics and you can see all that in the epic category in the right side column here of the created issues lies in the backlog page. 

Note: each issue that you create in your backlog should only correspond to one Epic.
to ensures that the work that your team is performing is aligned with the product vision and what the team is trying to build during the Sprint.

Jira helps it easy to filter and quickly see which items correspond to an epic by selecting an epic. it's very helpful and it's very easy to quickly identify which user stories are tied to the appropriate epic.

**so by this point we have populated our entire backlog or what's known as our product backlog that will be used to develop our app.**

we have created several epics that are linked to different user stories
tasks and bugs and we have color coded each epic so that they're easier to identify and filter.

Now let's take a look at how you can categorize your issues using what's called **LABELS**,

#### Labels to Categorize Issues 
>so it's easier for you to identify and search for them in your project.
and  later on you'll be able to filter out your issues based on these labels.

> [!NOTE] > What is a label?
A Label is a one piece of word of text to categorize the entities under the same. Here in JIRA, Such as user stories, tasks or bugs.

> **a one word piece of text to categorize similar user stories tasks or bugs.**
> 
	Jira doesn't currently let you type two separate words as a label so if you do have to use two separate words then use an underscore or a dash between the two words

 we'll label all user stories that are under our epics as follows for the epics.
 
![[Pasted image 20241111122307.png]]
![[Pasted image 20241111122428.png]]
![[Pasted image 20241111122456.png]]


How to label these issues all? 
backlog ->  Click User Story -> *opens a panel on the right* * -> 
scroll down to the details area you'll see a separate section called labels
-> Click -> Enter new one or assign existing one relevant to this Issue.

Tip: ==**keyboard shortcut 'L' that redirects to the selected issue to assign label**==
Label all the issues. after labelling all of our user stories with the correct label
We can quickly filter which issue is tied to the appropriate label by the Label option on top in the backlog section.
![[Pasted image 20241111125801.png]]
can able to easily filter each one with the appropriate label.


**Now, we have our complete list of backlog items with user stories, tasks and bugs tied to the appropriate labels and epics.**
...
#### How to use Components and Versions in Jira 
Jira -> Project -> *left panel* * -> Components 
it'll be completely empty. 
at last thing it will tell you about the compass component section but 
they also have one that's specific to Jira components and 
**you can see here that it's completely empty because we haven't created any components at this point in time** 

> [!NOTE] What are Components?
> Components are subsection of a project. 
> Use them to group issues within a project into smaller parts. Group and track work around components of your software architecture, cataloged in Compass.

components are very similar to labels.
**but to create them you do likely need to be an admin**, you should have the ability to create components for your project

**how you would use components**
eg: let's say that you're developing a software application. 
you could have two separate components or the different platforms the app would operate in. eg: iOS versus Android  or Windows versus Mac OS, based on the component type would mean that the issue will be assigned to the appropriate engineer to work on.

**so similar to labels you can use filters based on the components and see the issues which are tied to that component.**

**LETS CREATE ONE:** (on the latest version, the COMPONENTS has been developed as a separate product as Compass)
![[Pasted image 20241111131959.png]]
Compass: helps,
- Improve efficiency and accuracy with an automated, data-rich catalog of your software architecture.
- Get instant, actionable insights into the performance of your processes, products, and teams.
- Ensure quality, consistency, and transparency in your practices, standards, and work across your organization.
and
Unify, automate, and optimize your toolchain
#### Connect your source code tool to Compass

- Automate catalog creation and maintenance
- Enrich it with events, metrics, and other important data
- Enable all Compass full users to import repositories as components
from GIT PLATFORMS such as 
- Bitbucket
- GitHub
- GitLab
We'll install an app for your source code tool and help you connect it to Compass.  
You can always change the connection or connect other tools from the **Apps** page later.
![[Pasted image 20241111132346.png]]

so let's go ahead and create two components for our project first click on the create component button and you should see this dialogue box pop up for our first component let's call it iOS for description let's type in compatibility for iOS devices for component lead let's assign this to yourself so in my case I'll type in Alvin and then for default assigning let's select component lead and that way  it'll assign it to yourself click save and we'll now see that we've created our first component I'm going to ignore this popup for compus and instead we're going to focus only on the things tied to Jira 
