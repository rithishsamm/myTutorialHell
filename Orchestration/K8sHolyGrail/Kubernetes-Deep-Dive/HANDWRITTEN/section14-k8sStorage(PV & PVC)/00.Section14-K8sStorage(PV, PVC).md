...
- Storage
- AccessMode
Has to be identical in order to both of these to be bounded. 
...
Will deploy APPLICATION using `deployment` object attached to this PVC. 
Here,
- Names,
- Labels,
- Volume mount names,
Has to be identical with the metadata, selector, spec - templates, volumemount with volumes complied to the containers in order work in tandem with each other,  
...
With three replicas running - serves same content even from different PODs running on different nodes. Ok with the AccessMode - RWX, this is possible that is serves content. What if?

###### With ROX: (ReadOnlyMany)
Do all that but replace AccessMode - ReadOnlyMany - ROX. 
And define - 
```
volumes:
	- name: volumeName
	  persistentVolumeClaim:
	  	...
	  	readOnly: true
```
And verify if we can able to write any,

```
kubectl exec - it podName -- bash
```
Throws: cannot do that - `Read Only File System`.

Done with the AccessMode - RWX, RWO, ROX!

Perform cleanup and phew!
...
What are all the phases/status of PVs and PVCs we have, what means what.

PV Phases - tells what is the status of the PV's and PVC's that we create. 

A Volume will be in one of the following phases:
- **Available** - a free resource that/**which is not yet bound to a claim**,
This is what we generally see when we create one first hand, when a PVC gets deleted and reclaim the PV manually, when it is in retain reclaim policy. Available phase - means, this has been created and ready to be claimed, reclaimed if no PVC and used with the fresh or other PVC's with matching expressions by the VolumePlugins.

- **Bound** - the volume is bounded to a claim,
PV -> attached to -> PVC. Says which  PVC is bounded to which PV. 

- Released - the claim has been deleted, but the resources is not yet reclaimed by the cluster. 
Created a PV and attached to a PVC. After deleting a PVC when it is in the **`Retain` Reclaim policy**, - retains the PV having it standalone waiting to be reclaimed, attaching the Released PV with a PVC will be performed manually by the Administrator. 

- Failed - the volume has failed its automatic reclamation. 
PV Setup fails - due to misconfiguration, incompatible setup or such. Eg: nfs - supports - Retain, Recycle but not delete Reclaim Policy. If u perform deleting and tried the incompatible setup - Falls under failed phase. 

These are the four phases to know when we are working with PV or PVC concept. (especially PVs).
