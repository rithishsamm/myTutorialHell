Sv##### Section 13: K8s Storage (PV, PVC)
...
| ---------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Retain (default)                   | Even after deletion of the `PVC, PV` still exists with "Released" status, but it is ready for another claim (PVC).<br>Administrator has to manually reclaim the PV. We need to enable `Retain`, only when we want to `re-use` the PV. |
| Recycle (good for `hostPath, nfs`) | If Volume Plugin supports this feature, we can re-use PV, but looks like a new storage with no data (rm -rf /volume/*).<br><br>This reduces the overhead of administrator, where no manual reclaim is needed.                         |
| Delete                             |  If Volume Plugin supports this feature, deleting a PVC will delete the bound PV and backend Volume Plugin resource as well.                                                                                                          |
Note:
1) Reclaim Policy will be applied only for PV, not for PVC object 
2) We can modify/change Reclaim Policy by administrator after object creation using patch or edit. 
3) Best practice is to control the PV creation dynamically, where can define `Reclaim Policy` as template. 
