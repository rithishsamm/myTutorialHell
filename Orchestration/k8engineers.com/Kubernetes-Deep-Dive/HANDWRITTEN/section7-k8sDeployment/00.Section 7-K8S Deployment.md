...
Since we haven't declared in the manifest about all this `rollback`, `rollout` strategies and all. including the `StrategyType`, the type of `RollingUpdate` as `max unavailability` and `max surge`. Nothing has been mentioned over here in the manifest. THEN HOW TO PERFORM ALL THE SAME. 
