...
Understanding `matchLabels` + `matchExpressions` for ReplicaSet (NOT `rc` TO GET A BRIEF UNDERSTANDING ABOUT THE WORKINGS OF LABELS FOR `rc` AND `rs`):
...
Let play around by removing one of the label of `rs`, to do that,  run the same and pick one of the PODs Label:
```
kubectl label pods PodName
```
or
```
