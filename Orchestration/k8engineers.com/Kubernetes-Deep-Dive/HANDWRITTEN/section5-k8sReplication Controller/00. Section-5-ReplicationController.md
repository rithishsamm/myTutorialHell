...
`spec`: 
  ...
  `selector`: as we discussed earlier, this selector picks and identify the pods based on the labels of the pods defined in the manifests. THIS IS THE SELECTOR THAT THE `rc` is gonna use to identify the pods.
    `app: nginx-prod`: lets say that there is a pod defined under this label on the manifest's metadata.
