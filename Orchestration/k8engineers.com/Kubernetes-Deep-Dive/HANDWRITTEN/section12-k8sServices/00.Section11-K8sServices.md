...
HERE, WE ARE GOOD AND WILL PROCEED WITH THE DEFAULT. Come out without saving it. NEXT
...
here, using router network. This K8's Cluster's Network config that we setup is connected to router using bridged network with promiscuous mode on for all. 
...
Without this, absence of creating this, we cannot have/create a **LoadBalancer** object as a K8s Service. 
...
```
-- announcing from node "workernode125" with protocol "layer2 - which is controlled by `metallb-speaker`.
...
Thats how the Packet travels to and fro between the cliend and the PODs. 
Point is that -> 
...
Incase you want to expose the application to the outside world outside the cluster of the K8s from your Network to a specific port, use externalIP-svc.
...
Will see more examples of wherever this ExternalName svc type applies in a K8s Cluster Environment, - 
...
Directly it will be contacting -> the PodIP's in the backend.  
...
```
traceroute to headless-svc (10.244.55.163), 30 hops max, 60 byte packets
 1  192-168-0-124.calico-typha.calico-system.svc.cluster.local (192.168.0.124)  0.030 ms  0.004 ms  0.003 ms
 2  192.168.0.125 (192.168.0.125)  0.435 ms  0.363 ms  0.325 ms
 3  10-244-55-163.headless-svc.default.svc.cluster.local (10.244.55.163)  0.545 ms  0.512 ms  0.986 ms
```
Also check, PodIPs.

This particular traffic just flew through: 1) + 2) Workernode (where cni lies) -> `cni (calico)` -> resolves back to that host WorkerNode -> 3) WorkerNode (where the endpoint lies)-> 3) PodName -> PodIP 

```
kubectl get po -o wide
```
Shows where it is all coming from. 
>  This how the packet travels from client -> actual Pod.

Helps for troubleshooting and better understanding. When you are using with or without headless service. 

-> After all these, do the cleanup.

THIS IS ABOUT HEADLESS  `SVC` 's.
Now we will be digging more in to concepts such as K 8s types of storages, volumes and Statefulset. 

Covered the prerequisite to cover these by understanding deep about Headless services with selector. 

Headless = SvcDNS -> PodIPs, used mainly for **Statefulset** workload resources apps. 
