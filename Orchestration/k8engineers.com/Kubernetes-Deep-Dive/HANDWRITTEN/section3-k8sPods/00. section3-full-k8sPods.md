...
spec:
  container:
  - name: hexrapp
    image: nginx:1.27.2 #image:versiontag
    ports:
     - name: nginx-port
       containerPort: 80
       protocol: TCP
...
eg: here, we have a pod with two containers. one nginx which serves data, one alpine which writes data. each `workdir` gets mounted on system's voume mount.
