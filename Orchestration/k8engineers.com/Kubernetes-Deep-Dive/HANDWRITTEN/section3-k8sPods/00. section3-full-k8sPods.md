...
- Static POD manifest can be defined through **Filesystem-hosted** or **Web-Hosted**, will see such implementations further.
- Kubelet can read default directory `/etc/kubernetes/manifests`, If we place manifest file (either JSON or YAML) to create static POD for **Filesystem-hosted** approach.
- Kubelet can download manifest file from the by passing `--manifest-url=<URL>` argument for **Web-hosted** approach

---
####  16.Challenges of standalone POD applications
###### Challenges of Standalone Pod - [Doc](obsidian://open?vault=tutorialHell&file=Orchestration%2Fk8engineers.com%2FKubernetes-Deep-Dive%2Foffl-raw-docs%2Fkubernetes%20deepdive%2Fsec3-k8s-pods%2F11.Challenges%20of%20Standalone%20Pod.docx)
We previously saw what are **STATIC PODS**. Will see about Standalone PODS and its challenges.

- POD is a basic entity to run application on K8s Cluster
- POD works as a standalone application in K8s Cluster
- POD are not bind to specific node (unless we use **nodeName**, **nodeSelector**, **nodeAffinity** etc..,)
...
- We should always define containers in POD are application specific
