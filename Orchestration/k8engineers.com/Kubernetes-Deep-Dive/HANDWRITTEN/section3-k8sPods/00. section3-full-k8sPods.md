...
- `initContainers` will not be in running state. it will be shown as completed state. exactly like initContainers.
...
Once the process/job of pulling and storing the same gets completed, the **`initContainer`**'s status will be shown as and goes into completed state. IT IS A ONE TIME PROCESS AND AFTER IT GET COMPLETED, WE COULD ACCESS THE SAME.
...
###### writing the same in a .yaml file.
```YAML
