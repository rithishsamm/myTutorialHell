...
Writing the same on a `YAML` file -> pass it to `kubectl` -> that to the `kube-api-server`
Will see how to do that in practice.
...
Here, What to use for POD creation? check the same:

NOW, WRITING A `YAML` FILE:
```
apiVersion: v1
kind: Pod
metadata:
