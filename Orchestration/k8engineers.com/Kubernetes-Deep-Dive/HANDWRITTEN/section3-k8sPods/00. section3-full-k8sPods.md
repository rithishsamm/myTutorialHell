...
> HOW CRI CREATES CONTAINERS AND STUFF AND HOW WE GET THE RESPONSE OR ACKNOWLEDGEMENT.
...
`kubelet` --> will pass command to `containerd cri` , via `crictl` to create all the relevant `PODs`
6) after all `PODs` has been created, `containerd cri` --> will send back the **acknowledgement** to `kubelet`
...
10) `apiserver` will send that info to `kubelet` and
11) WE WILL RECEIVE THE CONSOLE OUTPUT THAT THE **PODS HAS BEEN CREATED, FAILED OR PENDING**
12) ALL DONE!
