...
Understanding `matchLabels` for ReplicaSet (TO GET A BRIEF UNDERSTANDING ABOUT THE WORKINGS OF LABELS FOR  `rs` when we worked with `rs` likewise): 
...
        emptyDir: {}
```
==Since we have not written the `selector`, this manifest couldn't get schema validated and deploy the application as we wish to!==. Without `selector` no `rc`.  should definitely pass at least one or multiple values as selector.

**NOT LIKE `rc`**. there we 
