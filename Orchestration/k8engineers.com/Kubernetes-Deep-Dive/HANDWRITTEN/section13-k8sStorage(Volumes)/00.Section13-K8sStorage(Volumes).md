...
**Critical points of `EmptyDir()`:**
![[emptyDir()]]

Nothing different. Exactly same as what we were doing to the `nginx-multicontainer` workload using `emptyDir()` VolumeType for volume which is an **Ephimeral VolumeType**
